...
const _assert = (globalThis as Record<string, unknown>).assert as (
  cond: unknown,
  msg?: string
) => void;
...
_describe('Slack skill', () => {
  ...
  _it('should get messages when connected', () => {
    freshInit();
    const result = _callTool('get_messages', { channel_id: 'C123' }) as
      | string
  ...
  _it('should send message when connected', () => {
    freshInit();
    const result = _callTool('send_message', { channel_id: 'C123', text: 'Hi' }) as
      | string
      | Record<string, unknown>;
    const parsed = typeof result === 'string' ? JSON.parse(result) : result;
    _assertEqual(parsed.ok, true, 'should be ok');
  ...
  _it('should return messages from DB', () => {
    freshInit();
    const result = _callTool('get_messages', { channel_id: 'C123', limit: 10 }) as
      | string
      | Record<string, unknown>;
    const parsed = typeof result === 'string' ? JSON.parse(result) : result;
    _assertEqual(parsed.ok, true, 'should be ok');
