...
var __skill_bundle = (() => {
 ...
 var BASE64_CHARS, BASE64_URL_CHARS, Buffer2;
 var init_buffer = __esm({
  "scripts/polyfills/buffer.js"() {
   init_buffer_inject();
   BASE64_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
   BASE64_URL_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";
 ...
 function publishSkillState() {
  ...
  state.setPartial({
   ...
   syncProgress: s2.syncStatus.syncProgress,
   syncProgressMessage: s2.syncStatus.syncProgressMessage || null,
   lastSyncTime: s2.syncStatus.lastSyncTime || null,
   nextSyncTime: s2.syncStatus.nextSyncTime || null,
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
   try {
    ...
    if (response.status === 401) {
     ...
     console.log(`[gmail] gmailFetch: 401 Unauthorized url=${url} credentialId=${credential.credentialId} body=${bodyPreview}`);
    ...
    if (response.status >= 200 && response.status < 300) {
     ...
     s2.lastApiError = error.message || `HTTP ${response.status}`;
 ...
 async function loadGmailProfile() {
  ...
  if (response.success) {
   ...
   console.log(`[gmail] Profile loaded for ${s2.profile.emailAddress}`);
  }
 }
...
${"".padEnd(offset)}${"^".repeat(len)}`;
 ...
 var Parser = class {
  ...
  oncdata(start2, endIndex, offset) {
   var _a2, _b, _c, _d, _e, _f, _g2, _h, _j, _k;
   this.endIndex = endIndex;
 ...
 function upsertEmail(message, redactSensitive = false) {
  ...
  const hasAttachments3 = hasEmailAttachments(message);
  ...
  const sensitive = isSensitiveText(subject) || isSensitiveText(bodyText || "") || isSensitiveText(message.snippet);
  if (sensitive && redactSensitive) {
   bodyText = "[Content redacted \u2014 contains sensitive information]";
   bodyHtml = null;
  }
  db.exec(`INSERT OR REPLACE INTO emails (
 ...
 function emailExists(id) {
  const cid = credId();
  return db.get("SELECT 1 FROM emails WHERE credential_id = ? AND id = ? LIMIT 1", [cid, id]) !== null;
 }
 ...
 function checkPart(part) {
  ...
  if (Array.isArray(part.parts))
   return part.parts.some((p) => checkPart(p));
  return false;
 }
 ...
 function insertEmailAttachments(message) {
  ...
  attachments.forEach((att) => {
   db.exec(`INSERT OR REPLACE INTO attachments
   (credential_id, message_id, attachment_id, filename, mime_type, size, part_id)
   VALUES (?, ?, ?, ?, ?, ?, ?)`, [cid, message.id, att.attachmentId, att.filename, att.mimeType, att.size, att.partId]);
  });
 }
 ...
 function initializeGmailSchema() {
  ...
  if (!columnNames.has("credential_id")) {
   ...
   const threadColNames = new Set(threadCols.map((col) => col.name));
   ...
   const labelColNames = new Set(labelCols.map((col) => col.name));
   ...
   const attColNames = new Set(attCols.map((col) => col.name));
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_emails_thread_id ON emails (credential_id, thread_id)", []);
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_attachments_message ON attachments (credential_id, message_id)", []);
 ...
 function emitSyncProgress(message, progress) {
  ...
  s2.syncStatus.syncProgressMessage = message;
  state.setPartial({ syncProgress: progress, syncProgressMessage: message });
 }
 function syncGmailMetadataToBackend() {
  const s2 = getGmailSkillState();
  if (!s2.profile)
   return;
  const metadata = {
 ...
 function gmailDateStr(msOrDaysAgo, isDaysAgo = false) {
  const ms = isDaysAgo ? Date.now() - msOrDaysAgo * 24 * 60 * 60 * 1e3 : msOrDaysAgo;
  const d = new Date(ms);
 ...
 async function performInitialSync(onProgress) {
  ...
  s2.syncStatus.totalEmails = getEmailCount();
  publishSkillState();
  try {
   ...
   s2.lastApiError = error instanceof Error ? error.message : String(error);
   emitSyncProgress(`Sync failed: ${s2.lastApiError}`, 0);
  } finally {
   s2.syncStatus.syncInProgress = false;
 ...
 function isSensitiveText2(text) {
 ...
 var getEmailTool = {
  ...
  async execute(args) {
   try {
    ...
    if (localEmail) {
     result.local_info = {
      created_at: new Date(localEmail.created_at * 1e3).toISOString(),
      updated_at: new Date(localEmail.updated_at * 1e3).toISOString()
     };
    }
    const s2 = getGmailSkillState();
 ...
 function buildListParams(args) {
  ...
  if (args.label_ids && Array.isArray(args.label_ids)) {
   args.label_ids.forEach((labelId) => {
 ...
 var getEmailsTool = {
  ...
  input_schema: {
   ...
   properties: {
    accessToken: {
     type: "string",
     description: "Optional OAuth access token. When provided (e.g. by frontend after OAuth), Gmail API is called directly with this token instead of the skill credential."
    },
    query: {
     type: "string",
 ...
 var getLabelsTool = {
  ...
  async execute(args) {
   try {
    ...
    if (!includeHidden) {
     labels = labels.filter((label) => label.labelListVisibility === "labelShow" || label.labelListVisibility === "labelShowIfUnread");
    }
    const formattedLabels = labels.map((label) => ({
     ...
     color: label.color ? { text: label.color.textColor, background: label.color.backgroundColor } : null
    }));
    labels.forEach((label) => upsertLabel(label));
    const categorized = {
     system: formattedLabels.filter((l2) => l2.type === "system"),
     user: formattedLabels.filter((l2) => l2.type === "user")
    };
    return JSON.stringify({
     success: true,
  ...
  async execute(args) {
   try {
    ...
    return JSON.stringify({
     ...
     successful: results.filter((r2) => r2.success).length,
 ...
 function getLabelOperations(action, labelIds = []) {
  const operations = {};
  switch (action) {
   case "mark_read":
 ...
 var searchEmailsTool = {
  ...
  async execute(args) {
   try {
    ...
    const maxResults = Math.min(parseInt(args.max_results || "20", 10), 100);
    params.push(`maxResults=${maxResults}`);
    if (args.include_spam_trash) {
     params.push("includeSpamTrash=true");
    }
 ...
 function calculateRelevanceScore(message, query) {
  ...
  const daysSinceMessage = (Date.now() - messageDate.getTime()) / (1e3 * 60 * 60 * 24);
  if (daysSinceMessage < 7) {
   score += 2;
  }
  return score;
 }
 function generateSearchTips(query) {
  const tips = [];
  if (!query.includes(":")) {
...
`;
     rawMessage += `References: <${args.reply_to_message_id}>\r
`;
    }
    rawMessage += `MIME-Version: 1.0\r
`;
    const attachments = args.attachments || [];
    if (attachments.length > 0) {
     rawMessage += `Content-Type: multipart/mixed; boundary="${boundary}"\r
\r
`;
    } else {
     if (bodyHtml && bodyText) {
      rawMessage += `Content-Type: multipart/alternative; boundary="${boundary}"\r
\r
`;
     } else if (bodyHtml) {
      rawMessage += `Content-Type: text/html; charset=utf-8\r
\r
`;
     } else {
      rawMessage += `Content-Type: text/plain; charset=utf-8\r
\r
`;
     }
    }
    if (attachments.length > 0 || bodyHtml && bodyText) {
     if (bodyText) {
      rawMessage += `--${boundary}\r
`;
      rawMessage += `Content-Type: text/plain; charset=utf-8\r
\r
`;
      rawMessage += `${bodyText}\r
\r
`;
     }
     if (bodyHtml) {
...
`;
     ...
     attachments.forEach((attachment) => {
      rawMessage += `--${boundary}\r
`;
      rawMessage += `Content-Type: ${attachment.mime_type}\r
`;
      rawMessage += `Content-Disposition: attachment; filename="${attachment.filename}"\r
`;
      rawMessage += `Content-Transfer-Encoding: base64\r
\r
`;
      rawMessage += `${attachment.data}\r
\r
`;
     });
     rawMessage += `--${boundary}--\r
`;
     ...
     rawMessage += `${bodyText || bodyHtml}\r
`;
    }
 ...
 async function init() {
  ...
  if (typeof lastSync === "number")
   s2.syncStatus.lastSyncTime = lastSync;
  if (typeof lastHistoryId === "string")
   s2.syncStatus.lastHistoryId = lastHistoryId;
  s2.syncStatus.totalEmails = getEmailCount();
 ...
 async function start() {
  ...
  const s2 = getGmailSkillState();
  const credential = oauth.getCredential();
  if (credential && s2.config.syncEnabled) {
   ...
   options: [
    {
     ...
     value: s2.config.syncEnabled
    },
    {
     name: "syncInterval",
     type: "select",
     label: "Sync Interval",
     value: s2.config.syncIntervalMinutes.toString(),
     options: [
    ...
    },
    {
     ...
     value: s2.config.maxEmailsPerSync.toString(),
     options: [
    ...
    },
    {
     ...
     value: s2.config.notifyOnNewEmails
    },
    {
 ...
 async function onSetOption(args) {
  ...
  switch (args.name) {
   case "syncEnabled":
    s2.config.syncEnabled = Boolean(args.value);
    if (s2.config.syncEnabled && credential) {
     const cronExpr = `0 */${s2.config.syncIntervalMinutes} * * * *`;
     cron.register("gmail-sync", cronExpr);
    } else {
     cron.unregister("gmail-sync");
    ...
    break;
   case "syncInterval":
    s2.config.syncIntervalMinutes = parseInt(args.value, 10);
    if (s2.config.syncEnabled && credential) {
     cron.unregister("gmail-sync");
     const cronExpr = `0 */${s2.config.syncIntervalMinutes} * * * *`;
     cron.register("gmail-sync", cronExpr);
   ...
   case "showSensitiveMessages":
    s2.config.showSensitiveMessages = Boolean(args.value);
    break;
  }
  state.set("config", s2.config);
  publishSkillState2();
