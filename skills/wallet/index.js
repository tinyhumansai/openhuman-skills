...
var __skill_bundle = (() => {
 ...
 async function onSetupStart() {
  ...
  return {
   step: {
    ...
    description: "Choose which blockchain networks to enable for balance checks. Your wallet address from the app will be used.",
    fields: [
     {
      name: "evm_networks",
      type: "multiselect",
 ...
 async function onSetupSubmit(args) {
  ...
  if (args.stepId === "networks") {
   const networks = Array.isArray(DEFAULT_NETWORKS) ? DEFAULT_NETWORKS : [];
   const evmSelected = args.values.evm_networks || [];
 ...
 // skills-ts-out/wallet/state.js
 var skillState = {
  config: { walletAddresses: [], networks: [] },
  isRunning: false
 };
 globalThis.__walletSkillState = skillState;
 globalThis.getWalletSkillState = function getWalletSkillState() {
  return globalThis.__walletSkillState;
 };

 // skills-ts-out/wallet/tools/list-wallets.js
 var listWalletsTool = {
  name: "list_wallets",
  description: "List all wallet addresses configured for this skill (derived from your mnemonic in the app).",
  input_schema: { type: "object", properties: {} },
  async execute() {
   const s = globalThis.getState();
   const wallets = s.config.walletAddresses.map((address, index) => ({
    index,
    chain_type: "evm",
    address,
    label: `Wallet ${index}`
 ...
 // skills-ts-out/wallet/tools/list-networks.js
 var ETH_MAINNET = {
  chain_id: "1",
  name: "Ethereum Mainnet",
  rpc_url: "https://eth.llamarpc.com",
 ...
 var listNetworksTool = {
  ...
  async execute() {
   ...
   if (!networks || networks.length === 0) {
    ...
    const state2 = globalThis.state;
    if (state2?.set)
     state2.set("config", s.config);
   }
 ...
 // skills-ts-out/wallet/tools/get-balance.js
 var ETH_MAINNET2 = {
  chain_id: "1",
  name: "Ethereum Mainnet",
  rpc_url: "https://eth.llamarpc.com",
 ...
 async function evmGetBalance(rpcUrl, address) {
  ...
  return JSON.stringify({ address, balance_wei: hexBalance, balance_eth: eth.toFixed(18), symbol: "ETH" }, null, 2);
 }
 var getBalanceTool = {
  name: "get_balance",
  description: "Get the balance of a wallet address on a specific network. Use list_wallets for addresses and list_networks for chain_id.",
  input_schema: {
   type: "object",
   properties: {
  ...
  async execute(args) {
   ...
   if (!address)
    return JSON.stringify({ error: "Missing required parameter: address" });
   if (!chainId)
    return JSON.stringify({ error: "Missing required parameter: chain_id" });
   let network = s.config.networks.find((n) => n.chain_id === chainId && n.chain_type === "evm");
   if (!network && chainId === "1") {
    s.config.networks = s.config.networks?.length ? s.config.networks : [];
    if (!s.config.networks.some((n) => n.chain_id === "1")) {
     s.config.networks = [ETH_MAINNET2, ...s.config.networks];
     const state2 = globalThis.state;
     if (state2?.set)
      state2.set("config", s.config);
    }
    network = ETH_MAINNET2;
   }
   if (!network) {
    return JSON.stringify({
 ...
 // skills-ts-out/wallet/index.js
 function ensureEthereumNetwork(networks) {
 ...
 var tools = [listWalletsTool, listNetworksTool, getBalanceTool];
 _g.tools = tools;
 var skill = {
  info: {
   id: "wallet",
