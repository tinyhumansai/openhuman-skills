#!/usr/bin/env tsx
/**
 * live-runner-node.ts - QuickJS Skill Script Runner with REAL network support (Node.js)
 *
 * Unlike the regular runner-node.ts which mocks network calls, this runner
 ...
 *
 * Usage:
 *   npx tsx dev/test-harness/live-runner-node.ts <skill-id> <script-file>
...
import { getMockState, initMockState } from './mock-state';
...
function printBanner(): void {
  ...
  console.log(`${colors.magenta}         QuickJS Skill Live Runner (Real Network)              ${colors.reset}`);
  console.log(`${colors.magenta}═══════════════════════════════════════════════════════════════${colors.reset}`);
}
...
${colors.yellow}Examples:${colors.reset}
  TELEGRAM_API_ID=12345 TELEGRAM_API_HASH=abc... npx tsx dev/test-harness/live-runner-node.ts telegram scripts/examples/test-telegram-flow.js
...
function createLiveBridgeAPIs(state: MockState): Record<string, unknown> {
  ...
  const platform = {
    os: (): string => process.platform === 'darwin' ? 'macos' : process.platform,
    env: (key: string): string => process.env[key] ?? '',
...
async function main(): Promise<void> {
  ...
  if (skillId === 'telegram') {
    ...
    if (session) {
      console.log(`${colors.green}✓${colors.reset} Session string provided`);
    } else {
